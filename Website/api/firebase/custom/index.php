...
<html>
    <head>
        <title>Envio de Notificaciones Firebase</title>
        <meta charset="UTF-8" />
    ...
    <body>
        ...
        // payload opcional
        $payload = array();
        $payload['expiry_date'] = '04/10/2016';
        $payload['discount'] = '0.2';
        ...
        // Tipo de notificacion - single user / topic
        $push_type = isset($_GET['push_type']) ? $_GET['push_type'] : '';

        // establecer si lleva imagen o no
        $include_image = isset($_GET['include_image']) ? TRUE : FALSE;

        $notification -> setTitle($title);
        $notification -> setMessage($message);
        if ($include_image) {
            $notification -> setImage('https://pixabay.com/static/uploads/photo/2016/07/27/00/58/oldtimer-1544342_960_720.jpg');
        } else {
